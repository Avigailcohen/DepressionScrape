...
DEPRES_NROWS = 10300  # number of rows to read from DEPRESSIVE_TWEETS_CSV
...
random_tweets_df = pd.read_csv(RANDOM_TWEETS_CSV, encoding="ISO-8859-1", usecols=["Index", "message to examine", "label (depression result)"], nrows=RANDOM_NROWS, header=0)
...
random_tweets_arr = [x for x in random_tweets_df['message to examine']]

X_d = clean_tweets(depressive_tweets_arr)
X_r = clean_tweets(random_tweets_arr)

tokenizer = Tokenizer(num_words=MAX_NB_WORDS)
tokenizer.fit_on_texts(X_d + X_r)#data from depressive and random tweets-to convert into sequences of integers
...
for (word, idx) in word_index.items():
    if idx >= MAX_NB_WORDS:
        continue  # Skip words with an index equal or greater than MAX_NB_WORDS
...
#the functions perm_d and perm_r-This step is crucial for shuffling the data before splitting it into training,
# validation, and test sets,
# ensuring that the model is trained on a representative and randomized sample of the data.
perm_d = np.random.permutation(len(data_d))#return an array of random numbers from 0 to the number of tweets-1
...
#model_save_path = r'C:\Users\AAA\OneDrive\מסמכים\GitHub\Depression_detection_using_Twitter_post\model.h5'
